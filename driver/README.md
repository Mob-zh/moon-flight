这个`driver`文件夹是飞控的**硬件驱动层目录**，负责直接与硬件交互，为上层模块（如`flight`、`fc`）提供统一的硬件操作接口，实现 “软件与硬件的解耦”。以下是各子模块 / 文件的核心作用分类：
...
| 文件 / 子模块                            | 作用                                                         |
| ---------------------------------------- | ------------------------------------------------------------ |
| `i2c_dev.c/h`、`spi_dev.c/h`         | I2C/SPI 总线驱动：实现传感器、Flash 等硬件的通信，是多数外设的基础通信方式（如 IMU 常用 I2C/SPI）。 |
...
| 文件             | 作用                                                         |
| ---------------- | ------------------------------------------------------------ |
| `dshot*.c/h`    | DShot 协议驱动：实现飞控与电调的数字通信（穿越机常用 DShot600，抗干扰强、响应快），是电机控制的核心接口。 |

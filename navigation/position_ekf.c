...
void position_ekf_update_baro(position_ekf_t *ekf, float altitude)
{
    ...
    // 气压计更新（用于高度估计）
    // 简化：只估计气压零偏，不直接更新水平位置

    float meas_alt = altitude;  // 相对高度

    // 卡尔曼增益（融合气压高度）
    float K = ekf->P[4] / (ekf->P[4] + ekf->r_baro);

    // 更新气压零偏（假设真实高度不变）
    // 这里简化处理：气压计主要用于垂直速度估计
    (void)K;  // 暂时不使用
...
{
    if (north) *north = ekf->pos_n;
    if (east) *east = ekf->pos_e;
    if (alt) *alt = 0.0f;  // 需要融合气压计才能获取准确高度
}

void position_ekf_get_velocity(position_ekf_t *ekf, float *vel_n, float *vel_e)
{
    if (vel_n) *vel_n = ekf->vel_n;
    if (vel_e) *vel_e = ekf->vel_e;
}
